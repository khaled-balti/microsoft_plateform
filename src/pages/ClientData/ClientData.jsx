...
const ClientData = () => {
  ...
  const percentage = (addedClients && allClients) ? Math.round((addedClients / allClients) * 100) : 0;
  ...
  const deleteClientHandler = async(id) => {
    try {
      ...
      if (order) {
        toast.error("you can't detete this client, it is associated with orders")
      }
      else {
  ...
  const actionColumn = [
    {
      ...
      renderCell: (params) => (
        <div className="d-flex justify-content-between align-items-center">
          ...
          <div className='text-danger' style={{ cursor: 'pointer' }} onClick={() => deleteClientHandler(params.row.id)} >
            <MdDelete size={25} />
          </div>
        </div>
      ),
        ...
        <div className='row py-3 px-3'>
          <div className='col-12 col-md-6 col-lg-4 ps-lg-0 mb-2 mb-lg-0'>
            <div className='col-12 data-core d-flex flex-column justify-content-between align-items-center py-4' style={{ height: '300px' }}>
              ...
              <p className='fs-3 fw-semibold mt-4'>Total Clients</p>
              <p className='fw-bold' style={{ fontSize: '50px' }}>{allClients}</p>
            </div>
          </div>
          <div className='col-12 col-md-6 col-lg-4 mb-2 mb-lg-0'>
            <div className='col-12 data-core d-flex flex-column justify-content-center align-items-center py-4' style={{ height: "300px" }}>
              ...
              <p className='mt-3 fs-5 fw-semibold'>Clients Percentage</p>
            </div>
          </div>
          <div className='col-12 col-md-6 col-lg-4 pe-lg-0 mb-2 mb-lg-0'>
            <div className='col-12 data-core d-flex flex-column justify-content-between align-items-center py-4' style={{ height: '300px' }}>
              ...
              <p className='fs-3 fw-semibold mt-4'>Added Clients</p>
              <p className='fw-bold' style={{ fontSize: '50px' }}>{addedClients}</p>
            </div>
          </div>
        </div>
      )}
      <div className='container data-core w-100 py-3 px-4'>
        <div className='d-flex justify-content-between align-items-center mb-5' style={{ height: '40px' }}>
          <div className='search d-flex justify-content-between align-items-center px-2 w-75'>
            <input type='text' placeholder='search by client name...' className='input' onChange={changeInputHandler} />
            <FaSearch size={22} />
          </div>
          {(role === "admin" || role === "superadmin") && (
            <Link to="/dashboard/data/client/add" end style={{ height: '100%' }}>
